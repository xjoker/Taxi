        ...
        private void FileWrite(LogType log)
        {
            try
            {
                ...
                if (_writer != null)
                {
                    ...
                    // 日志文件模板
                    _writer.WriteLine("记录时间:" + log.DateTime);
                    _writer.WriteLine("日志级别：" + log.LogLevel.ToString());
                    _writer.WriteLine("出错类：" + log.ErrorClass);
                    _writer.WriteLine("行号：" + log.LineNumber.ToString());
                    _writer.WriteLine("错误描述：" + log.ErrorMessage);
                    _writer.WriteLine("错误详细：" + log.ErrorDetails);
                    _writer.WriteLine(" ");
                    _writer.WriteLine("--------------------------------------------------------------");
                    _writer.WriteLine(" ");
